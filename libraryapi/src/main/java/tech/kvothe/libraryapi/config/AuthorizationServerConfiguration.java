        ...
        return context -> {
            ...
            if (principal instanceof CustomAuthentication customAuthentication) {
                ...
                if (OAuth2TokenType.ACCESS_TOKEN.equals(tokenType)) {
                    var authorities = customAuthentication.getAuthorities();
                    List<String> autoritiesList = authorities.stream().map(GrantedAuthority::getAuthority).toList();
                    context.getClaims()
                            .claim("authorities", autoritiesList)
